...
export default function LeftSidebar({ isOpen, onClose }: LeftSidebarProps) {
      ...
      <nav className="flex-1 py-4 px-3 space-y-6 overflow-y-auto">
        {sidebarNavigation.map((section, sectionIndex) => (
          <div key={sectionIndex}>
            {section.title && (
              <h3 className="px-3 mb-2 text-xs font-semibold text-[var(--text-tertiary)] uppercase tracking-wider">
                {section.title}
              </h3>
            )}
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <aside className="hidden lg:flex flex-col w-64 h-[calc(100vh-4rem)] sticky top-16 border-r border-[var(--border-light)] bg-[var(--bg-primary)]">
        <SidebarContent />
      </aside>
